...
async def test_preparation(context: MemoryContext) -> str:
    ...
    url = f"{URL_RAW}tasks/{data['test_id']}.json"
...
async def test_result(data: dict[str, Any]) -> None:
    ...
    async with ClientSession() as session:
        async with session.get(URL_API, headers=HEADERS) as response:
            response = loads(await response.read())
            content = b64decode(response["content"]).decode()
            results = loads(content) if content else {}
            (
                results.setdefault(data["group_id"], {})
        ...
        async with session.put(
            ...
            json={
                "message": "+=",
                "content": b64encode(dumps(results)).decode(),
                "sha": response["sha"],
            },
            headers=HEADERS,
        ) as _:
